...
document.addEventListener('DOMContentLoaded', function (e) {
  ...
  const dt_faq_table = document.querySelector('.datatables-faq'),
    ...
    faqGetList = baseUrl + 'app/faq/get-list',
    statusObj = {
    ...
    var dt_faq = new DataTable(dt_faq_table, {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          targets: 4,
          render: function (data, type, full, meta) {
            // Cek apakah ada data relasi layanan
            const layanan = full.layanan && full.layanan.nama 
              ? full.layanan.nama 
              : (full.layanan_id ? `ID: ${full.layanan_id}` : 'N/A');
            return `<span>${layanan}</span>`;
          }
        },
        {
          // Status
          ...
          render: function (data, type, full, meta) {
            return `
              <div class="d-inline-block text-nowrap">
                <button class="btn btn-text-secondary rounded-pill waves-effect btn-icon view-faq" data-id="${full.id}">
                ...
                <div class="dropdown-menu dropdown-menu-end m-0">
                  <a href="javascript:void(0);" class="dropdown-item delete-faq" data-id="${full.id}">Hapus</a>
                </div>
              </div>
            `;
          }
