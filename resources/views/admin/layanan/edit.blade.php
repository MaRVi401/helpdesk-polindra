...
<body>
    <div class="main-container">
        ...
        <form action="{{ route('admin.layanan.update', $layanan) }}" method="POST">
                ...
                <select name="unit_id" id="unit_id" required>
                    @foreach ($units as $unit)
                        <option value="{{ $unit->id }}"
                            {{ old('unit_id', $layanan->unit_id) == $unit->id ? 'selected' : '' }}>
                            {{ $unit->nama_unit }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name="status_arsip" id="status_arsip" required>
                    <option value="0" {{ old('status_arsip', $layanan->status_arsip) == 0 ? 'selected' : '' }}>
                        Tidak Diarsip (Aktif)</option>
                    <option value="1" {{ old('status_arsip', $layanan->status_arsip) == 1 ? 'selected' : '' }}>
                        Diarsip (Non-aktif)</option>
                </select>
            </div>
                ...
                <select name="pic_id_to_add" style="flex-grow: 1;" required>
                    ...
                    @forelse($availableStaff as $staff)
                        <option value="{{ $staff->id }}">
                            {{-- Format diseragamkan: Nama (NIK: [NIK]) --}}
        ...
        <ul class="pic-list">
            @forelse($currentPICS as $pic)
                <li class="pic-item">
                    <div class="pic-info">
                        {{-- Format diseragamkan: Nama (NIK: [NIK]) --}}
                        {{ $pic->user->name ?? 'N/A' }} (NIK: {{ $pic->nik ?? 'N/A' }})
                    </div>
                    <div class="pic-actions">
                        <form action="{{ route('admin.layanan.update', $layanan) }}" method="POST"
                            ...
                            <button type="submit" class="button button-danger"
                                onclick="return confirm('Anda yakin ingin menghapus PIC ini?')">Hapus</button>
                        </form>
                    </div>
                </li>
            @empty
        ...
        <div class="form-actions">
            <a href="{{ route('admin.layanan.index') }}" class="button button-secondary">Kembali ke Daftar
                Layanan</a>
        </div>
    </div>
</body>
