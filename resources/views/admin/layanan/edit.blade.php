    ...
    <div class="main-container">
        ...
        <form action="{{ route('admin.layanan.update', $layanan) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="number" name="prioritas" id="prioritas" value="{{ old('prioritas', $layanan->prioritas) }}" min="0">
            </div>
            ...
            <div class="form-group">
                ...
                <select name="penanggung_jawab_ids[]" id="penanggung_jawab_ids" multiple size="8">
                    @foreach($allStaff as $staff)
                        {{-- Logika untuk 'selected' disesuaikan di sini --}}
                        <option value="{{ $staff->id }}" {{ $assignedPicIds->contains($staff->id) ? 'selected' : '' }}>
            ...
            <div class="form-group form-group-check">
                <input type="checkbox" name="status_arsip" id="status_arsip" value="1" {{ old('status_arsip', $layanan->status_arsip) ? 'checked' : '' }}>
                <label for="status_arsip" style="margin-bottom: 0;">Arsipkan layanan ini (sembunyikan dari mahasiswa)</label>
            </div>

            <div class="form-actions">
                <button type="submit" class="button button-primary">Perbarui Layanan</button>
