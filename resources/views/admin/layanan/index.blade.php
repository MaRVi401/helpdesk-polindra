...
<head>
    ...
    <style>
        ...
        .main-container { background-color: white; padding: 32px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); width: 100%; max-width: 1000px; margin: 0 auto; }
        h1 { margin-top: 0; }
        ...
        .pic-list { margin: 0; padding-left: 1.2rem; }
    </style>
</head>
<body>
    <div class="main-container">
        ...
        <div class="toolbar">
            ...
            <form id="search-form" action="{{ route('admin.layanan.index') }}" method="GET" class="search-form">
                <select name="per_page" onchange="this.form.submit()">
                    <option value="10" {{ $perPage == 10 ? 'selected' : '' }}>10</option>
                    <option value="25" {{ $perPage == 25 ? 'selected' : '' }}>25</option>
                    <option value="50" {{ $perPage == 50 ? 'selected' : '' }}>50</option>
                </select>
                <input type="search" id="search-input" name="q" placeholder="Cari layanan, unit, atau PIC..." value="{{ $searchQuery ?? '' }}">
            </form>
        </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="action-buttons">
                        ...
                        <form action="{{ route('admin.layanan.destroy', $layanan) }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin ingin menghapus layanan ini?');">
                            @csrf
                            @method('DELETE')
