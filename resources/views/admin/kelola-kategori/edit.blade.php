...
<body>
    <div class="form-container">
        <h1>Edit Kategori</h1>
        <form action="{{ route('admin.kategori-artikel.update', $kategoriArtikel->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="kategori">Nama Kategori</label>
                <input type="text" id="kategori" name="kategori" value="{{ old('kategori', $kategoriArtikel->kategori) }}" required>
                @error('kategori') <p class="error-message">{{ $message }}</p> @enderror
            </div>
            <button type="submit" class="button">Update</button>
            <a href="{{ route('admin.kategori-artikel.index') }}" style="margin-left: 10px;">Batal</a>
        </form>
    </div>
</body>
</html>
