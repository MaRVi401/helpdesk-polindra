    ...
    <div class="container">
        ...
        <div class="toolbar">
            <div class="actions-group">
                <a href="{{ route('admin.manage-faq.create') }}" class="button">Tambah FAQ</a>
                <button type="button" id="export-button" class="button button-green" disabled>Ekspor Terpilih</button>
            </div>

            <form action="{{ route('admin.manage-faq.index') }}" method="GET" class="search-form" id="search-form">
                ...
                <input type="search" name="q" id="search-input" placeholder="Cari judul..."
                    value="{{ $searchQuery ?? '' }}">
            </form>
        </div>
        ...
        <table>
            ...
            <tbody>
                @forelse($faqs as $faq)
                    <tr>
                        <td><input type="checkbox" name="selected_faqs[]" class="faq-checkbox"
                                value="{{ $faq->id }}"></td>
                        ...
                        <td class="actions">
                            <a href="{{ route('admin.manage-faq.edit', $faq->id) }}">Edit</a>
                            <form action="{{ route('admin.manage-faq.destroy', $faq->id) }}" method="POST"
        ...
        <div class="pagination-container">
            <div>
                Menampilkan {{ $faqs->firstItem() ?? 0 }} sampai {{ $faqs->lastItem() ?? 0 }} dari
                {{ $faqs->total() }} total data
            </div>
            <div>
                {{ $faqs->links() }}
            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (exportButton) {
                exportButton.addEventListener('click', function() {
                    const selectedIds = Array.from(document.querySelectorAll('.faq-checkbox:checked'))
                        .map(cb => cb.value);

                    if (selectedIds.length > 0) {
                        const baseUrl = '{{ route('admin.kelolafaq.export.excel') }}';
                        const queryString = selectedIds.map(id => `selected_faqs[]=${id}`).join('&');
