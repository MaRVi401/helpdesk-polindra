    ...
    <div class="container">
        ...
        <form action="{{ route('admin.manage-faq.update', $faq->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" id="judul" name="judul" value="{{ old('judul', $faq->judul) }}" class="@error('judul') is-invalid @enderror" required>
                @error('judul')
            ...
            <div class="form-group">
                ...
                <select id="layanan_id" name="layanan_id" class="@error('layanan_id') is-invalid @enderror" required>
                    @foreach($layanans as $layanan)
                        <option value="{{ $layanan->id }}" {{ old('layanan_id', $faq->layanan_id) == $layanan->id ? 'selected' : '' }}>{{ $layanan->nama }}</option>
                    @endforeach
                </select>
                @error('layanan_id')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="form-group">
                ...
                <textarea id="deskripsi" name="deskripsi" class="@error('deskripsi') is-invalid @enderror" required>{{ old('deskripsi', $faq->deskripsi) }}</textarea>
                @error('deskripsi')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="status" name="status" class="@error('status') is-invalid @enderror" required>
                    ...
                    <option value="Post" {{ old('status', $faq->status) == 'Post' ? 'selected' : '' }}>Post</option>
                </select>
                @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="button-group">
                ...
                <a href="{{ route('admin.manage-faq.index') }}" class="button button-secondary">Batal</a>
            </div>
        </form>
    </div>
</body>
