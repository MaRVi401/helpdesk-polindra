...
@section('content')
  <div class="card">
    <h5 class="card-header">Edit Program Studi</h5>
    <div class="card-body">
      <form action="{{ route('study-program.update', $data_program_studi->id) }}" method="POST">
        @csrf
        @method('PUT')
        {{-- NAMA PROGRAM STUDI --}}
        <div class="mb-6">
          ...
          <input type="text" class="form-control @error('program_studi') is-invalid @enderror" id="program_studi"
            name="program_studi" placeholder="contoh: D4 - Rekayasa Perangkat Lunak" required value="{{ old('program_studi', $data_program_studi->program_studi) }}" />
          @error('program_studi')
        ...
        <div class="mb-6">
          ...
          <select class="form-select @error('jurusan_id') is-invalid @enderror" id="jurusan_id" name="jurusan_id"
            ...
            @foreach ($data_jurusan as $jurusan)
              <option value="{{ $jurusan->id }}" {{ old('jurusan_id', $data_program_studi->jurusan_id) == $jurusan->id ? 'selected' : '' }}>
