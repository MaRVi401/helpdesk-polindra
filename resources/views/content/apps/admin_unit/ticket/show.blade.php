...
{{-- Asumsi vendor styles sudah di-load di layout master, jika belum silakan tambahkan di sini --}}
...
{{-- INI ADALAH TAG PEMBUKA YANG HILANG --}}
@section('content')
...
<style>
    ...
    .dot-system { background-color: #a0aec0 !important; }
...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    @php
        ...
        $prioLabel = match ($prioVal) { 3 => 'Tinggi', 2 => 'Sedang', default => 'Rendah', };
        ...
        $picStatus = $tiket->riwayatStatus->where('status', 'Ditangani_oleh_PIC')->last() ?? 
    ...
    <div class="row g-4">
        ...
        {{-- KOLOM KIRI (Tindak Lanjut, Diskusi & Riwayat) - ORDER 2 di desktop --}}
        <div class="col-lg-8 col-md-12 order-lg-1 order-2">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    @if ($isFormDisabled)
                        @php
                            $alertClass = ($currentStatus === 'Diselesaikan_oleh_PIC') ? 'alert-info' : 'alert-success';
                            $iconClass = ($currentStatus === 'Diselesaikan_oleh_PIC') ? 'tabler-info-circle' : 'tabler-circle-check';
                        @endphp
                        <div class="alert {{ $alertClass }} d-flex align-items-center mb-3" role="alert">
                            {{-- Menggunakan class Tabler icon dari Vite --}}
                    ...
                    <form id="form-update-status" action="{{ route('admin_unit.ticket.update', $tiket->id) }}" method="POST">
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                @if(!$isFormDisabled)
                                    <div class="form-text text-warning">Pilih status berikutnya untuk memproses tiket ini.</div>
                                @endif
                            </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    {{-- Form Komentar --}}
                    @php
                        $isCommentDisabled = in_array($currentStatus, ['Dinilai_Selesai_oleh_Kepala', 'Dinilai_Selesai_oleh_Pemohon', 'Ditolak']);
                    @endphp
                    <form action="{{ route('admin_unit.ticket.comment', $tiket->id) }}" method="POST" class="mb-5"> 
                        ...
                        <div class="text-end">
                            <button type="submit" class="btn btn-secondary" {{ $isCommentDisabled ? 'disabled' : '' }}>
                    ...
                    <div class="timeline">
                        @forelse ($tiket->riwayatStatus->sortByDesc('created_at') as $riwayat)
                            ...
                            <div class="timeline-item">
                                ...
                                <div class="timeline-content">
                                    <div class="timeline-time">{{ $riwayat->created_at->translatedFormat('d F Y, H:i') }}</div>
                                    <div class="timeline-title">
        ...
        {{-- KOLOM KANAN (Sidebar) - ORDER 1 di desktop --}}
                ...
                <div class="card mb-4 border border-warning shadow-sm">
                    ...
                    <div class="card-body pt-3 text-center">
                        ...
                        <div id="admin-countdown" class="alert alert-warning p-2 mb-3 fw-bold" 
                            data-deadline="{{ \Carbon\Carbon::parse($deadline)->format('Y-m-d H:i:s') }}" style="font-size: 1.2rem;">
                            Memuat...
                        </div>
                        ...
                        <div class="collapse" id="settingTimer">
                            <form action="{{ route('admin_unit.ticket.updateTimer', $tiket->id) }}" method="POST" class="bg-light p-2 rounded border">
                                @csrf
                                @method('PUT')
                    ...
                    <div class="card-body">
                        ...
                        <div class="info-grid">
                            ...
                            @if (Str::contains($tiket->layanan->nama, 'Surat Keterangan Aktif Kuliah'))
                                <dt>Keperluan</dt><dd class="text-break">{{ $detailLayanan->keperluan ?? '-' }}</dd>
                                ...
                                @if ($detailLayanan->keperluan_lainnya ?? false)
                                    ...
                                    <dd class="col-12 border-top pt-2 text-break">{{ $detailLayanan->keperluan_lainnya }}</dd>
                                @endif

                            {{-- 2. Reset Akun --}}
                            @elseif(Str::contains($tiket->layanan->nama, 'Reset Akun'))
                                <dt>Aplikasi</dt><dd>{{ $detailLayanan->aplikasi ?? '-' }}</dd>
                                <dt>Deskripsi Masalah</dt><dd class="text-break">{{ $detailLayanan->deskripsi ?? '-' }}</dd>

                            {{-- 3. Ubah Data Mahasiswa --}}
                            @elseif(Str::contains($tiket->layanan->nama, 'Ubah Data Mahasiswa'))
                                <dt>Nama Lengkap</dt><dd>{{ $detailLayanan->data_nama_lengkap ?? '-' }}</dd>
                                <dt>Tempat Lahir Baru</dt><dd>{{ $detailLayanan->data_tmp_lahir ?? '-' }}</dd>
                                <dt>Tanggal Lahir Baru</dt><dd>{{ $detailLayanan->data_tgl_lhr ? \Carbon\Carbon::parse($detailLayanan->data_tgl_lhr)->format('d-m-Y') : '-' }}</dd>

                            {{-- 4. Request Publikasi --}}
                            @elseif(Str::contains($tiket->layanan->nama, 'Request Publikasi') || Str::contains($tiket->layanan->nama, 'Publikasi'))
                                <dt>Judul / Topik</dt><dd class="text-break">{{ $detailLayanan->judul ?? '-' }}</dd>
